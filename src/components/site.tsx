  ...
  const header = (
    <div className="flex items-center justify-between gap-4">
      <Link href={localizeHref("/", locale)} className="flex items-center gap-3">
        <div aria-hidden="true" className="relative grid h-9 w-9 place-items-center overflow-hidden rounded-xl border border-[hsl(var(--nav-theme))] bg-[hsl(var(--nav-theme))] shadow-sm">
          <span className="text-sm font-extrabold tracking-wide text-white">{siteConfig.logoText}</span>
        </div>
  ...
  return (
    <div className="group relative cursor-pointer overflow-hidden rounded-2xl border border-border shadow-lg transition-all duration-200">
      <div className="relative aspect-video w-full">
        <img src={videoId ? `https://i.ytimg.com/vi/${videoId}/maxresdefault.jpg` : "/images/hero.webp"} alt={`${siteConfig.shortName} Roblox gameplay preview`} className="size-full object-cover transition-all duration-200 group-hover:brightness-80" onError={(e) => { (e.target as HTMLImageElement).src = "/images/hero.webp"; }} />
      </div>
